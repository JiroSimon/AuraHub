...
class _PPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size sz) {
    for (final p in ps) {
      // wrap-around float upward with horizontal drift
      double y = ((p.y + p.vy * t * 120) % 1.0 + 1.0) % 1.0;
      // invert so particles float UP
      y = 1.0 - y;
  ...
  void _star(Canvas c, double s, Paint p) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (i == 0) path.moveTo(s * math.cos(oa), s * math.sin(oa));
      else path.lineTo(s * math.cos(oa), s * math.sin(oa));
      path.lineTo(s * 0.4 * math.cos(ia), s * 0.4 * math.sin(ia));
    }
  ...
  void _heart(Canvas c, double s, Paint p) {
    ...
    final path = Path();
    path.moveTo(0, r * 0.3);
    path.cubicTo(0, -r * 0.5, -r * 1.2, -r * 0.5, -r * 1.2, r * 0.3);
    path.cubicTo(-r * 1.2, r * 0.9, 0, r * 1.4, 0, r * 1.4);
    path.cubicTo(0, r * 1.4, r * 1.2, r * 0.9, r * 1.2, r * 0.3);
    path.cubicTo(r * 1.2, -r * 0.5, 0, -r * 0.5, 0, r * 0.3);
    c.drawPath(path, p);
  }

  void _bolt(Canvas c, double s, Paint p) {
    final path = Path();
    path.moveTo(0, -s); path.lineTo(-s * 0.4, 0);
    path.lineTo(s * 0.15, 0); path.lineTo(0, s);
    path.lineTo(s * 0.4, 0); path.lineTo(-s * 0.15, 0);
    path.close(); c.drawPath(path, p);
  }
  ...
  void _drop(Canvas c, double s, Paint p) {
    final path = Path();
    path.moveTo(0, -s);
    path.cubicTo(s * 0.8, -s * 0.2, s * 0.8, s * 0.5, 0, s);
    path.cubicTo(-s * 0.8, s * 0.5, -s * 0.8, -s * 0.2, 0, -s);
    c.drawPath(path, p);
  }

  @override bool shouldRepaint(_PPainter o) => true;
}

/* ===================== WAVEFORM BARS ===================== */
// Animated equalizer bars shown in Now Playing
        ...
        child: SafeArea(child: FadeTransition(opacity: _fa,
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Center(child: Column(children: [
                  Container(width: 80, height: 80,
                      decoration: BoxDecoration(shape: BoxShape.circle,
                          ...
                          boxShadow: [BoxShadow(color: const Color(0xFFFF1493).withValues(alpha: 0.5),
                              blurRadius: 36, spreadRadius: 4)]),
                      child: const Icon(Icons.music_note_rounded, color: Colors.white, size: 40)),
                  const SizedBox(height: 16),
                  const Text('AURAHUB', style: TextStyle(color: Colors.white, fontSize: 28,
                      fontWeight: FontWeight.w800, letterSpacing: 8)),
                  const SizedBox(height: 6),
                  const Text('feel the music', style: TextStyle(
                      color: Colors.white38, fontSize: 13, letterSpacing: 4)),
                ])),
                ...
                _Field(ctrl: _eCtrl, hint: 'Email address', icon: Icons.email_outlined,
                    type: TextInputType.emailAddress),
                const SizedBox(height: 14),
                _Field(ctrl: _pCtrl, hint: 'Password', icon: Icons.lock_outline_rounded, obs: _obs,
        ...
        child: SafeArea(child: FadeTransition(opacity: _fa,
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _Field(ctrl: _nCtrl, hint: 'Full name', icon: Icons.person_outline_rounded),
                const SizedBox(height: 14),
                _Field(ctrl: _eCtrl, hint: 'Email address', icon: Icons.email_outlined,
                    type: TextInputType.emailAddress),
                const SizedBox(height: 14),
                _Field(ctrl: _pCtrl, hint: 'Password', icon: Icons.lock_outline_rounded, obs: _obs,
                    suffix: IconButton(icon: Icon(_obs ? Icons.visibility_off_outlined : Icons.visibility_outlined,
                        color: Colors.white38, size: 20), onPressed: () => setState(() => _obs = !_obs))),
                const SizedBox(height: 14),
                _Field(ctrl: _cCtrl, hint: 'Confirm password', icon: Icons.lock_outline_rounded, obs: _obsC,
...
class _AuraHomePageState extends State<AuraHomePage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // FIX: single listener on the global player
    globalPlayer.playerStateStream.listen((state) {
      if (!mounted) return;
      setState(() => _playing = state.playing);

      // AUTOPLAY: completed → next
      if (state.processingState == ProcessingState.completed) {
        _playSong((_curIdx + 1) % kSongs.length);
      }
    });
  }
  ...
  @override
  void dispose() {
    _searchCtrl.dispose(); _tabCtrl.dispose(); super.dispose();
  }
  ...
  // FIX: stop → setAudioSource → play — no "already exists" crash
  Future<void> _playSong(int index) async {
    if (index < 0 || index >= kSongs.length) return;
    setState(() { _curIdx = index; _curMood = kSongs[index]['mood']!; });
    try {
      await globalPlayer.stop();
      await globalPlayer.setAudioSource(AudioSource.asset(kSongs[index]['path']!));
      await globalPlayer.play();
    } catch (_) {}
  }

  void _next() => _playSong((_curIdx + 1) % kSongs.length);
  void _prev() => _playSong((_curIdx - 1 + kSongs.length) % kSongs.length);
        ...
        actions: [
          ...
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.white,
              ...
              onPressed: () {
                ...
                if (name.isNotEmpty) {
                  setState(() => _playlists.add(Playlist(
  ...
  void _addToPlaylistSheet(Map<String, String> song) {
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (_playlists.isEmpty)
                const Padding(padding: EdgeInsets.symmetric(vertical: 24),
                    child: Center(child: Text('No playlists yet. Create one!', style: TextStyle(color: Colors.white38))))
              else ...(_playlists.map((pl) {
                final has = pl.songs.any((s) => s['title'] == song['title']);
                return ListTile(contentPadding: EdgeInsets.zero,
                    ...
                    trailing: has ? const Icon(Icons.check_circle_rounded, color: Colors.greenAccent, size: 22)
                        : const Icon(Icons.add_circle_outline_rounded, color: Colors.white54, size: 22),
                    onTap: () {
                      if (!has) {
                        setState(() => pl.songs.add(song)); set(() {});
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: Text('Added to ${pl.name}'), backgroundColor: const Color(0xFF2A2A2A),
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.redAccent,
              ...
              onPressed: () { setState(() => _playlists.remove(pl)); Navigator.pop(context); },
  ...
  void _openNowPlaying(List<Map<String, String>> list, int index) {
    Navigator.push(context, PageRouteBuilder(
        pageBuilder: (_, a1, a2) => NowPlayingPage(
            ...
            initialIndex: index,
            globalIdx: _curIdx,
            onIndexChanged: (newGlobalIdx) {
              if (mounted) setState(() {
                _curIdx = newGlobalIdx;
                _curMood = kSongs[newGlobalIdx]['mood']!;
              });
            }),
  ...
  Widget build(BuildContext context) {
    ...
    final curSong = kSongs[_curIdx];
    final curColors = mood(_curMood).gradient;
      ...
      body: Stack(children: [
        ...
        AnimatedContainer(duration: const Duration(milliseconds: 700), curve: Curves.easeInOut,
            decoration: BoxDecoration(gradient: LinearGradient(
                begin: Alignment.topLeft, end: Alignment.bottomRight,
                colors: _playing ? [curColors[0], curColors[1]]
                    : [const Color(0xFF0A0A0A), const Color(0xFF1A1A1A)]))),

        // PARTICLES
        MoodParticles(moodName: _curMood, active: _playing),
        ...
        Column(children: [
            ...
            Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 0),
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Text('feel the music', style: TextStyle(
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
                child: Container(
                    ...
                    child: TabBar(controller: _tabCtrl,
                        ...
                        tabs: const [Tab(text: 'Songs'), Tab(text: 'Playlists')]))),

            const SizedBox(height: 12),

            Expanded(child: TabBarView(controller: _tabCtrl, children: [
              // SONGS TAB
              _SongsTab(filtered: filtered, songs: kSongs, selMood: _selMood,
                  searchCtrl: _searchCtrl, curIdx: _curIdx, player: globalPlayer,
                  moodIcon: _moodIcon,
                  onMoodChange: (m) => setState(() => _selMood = m),
                  onSearch: (v) => setState(() => _searchQ = v),
                  onTap: (i, s) { _playSong(i); _openNowPlaying(kSongs, i); },
                  onLongPress: _addToPlaylistSheet),

              // PLAYLISTS TAB
              _PlaylistsTab(playlists: _playlists, player: globalPlayer,
                  onCreate: _createPlaylistDialog, onDelete: _deletePlaylist,
                  onPlay: (list, i) {
                    final gi = kSongs.indexWhere((s) => s['title'] == list[i]['title']);
                    if (gi != -1) _playSong(gi);
                    _openNowPlaying(list, i);
                  }),
            ])),
          ]))),
          ...
          StreamBuilder<Duration?>(
              stream: globalPlayer.durationStream,
              builder: (_, __) {
                if (globalPlayer.processingState == ProcessingState.idle) return const SizedBox.shrink();
                return MiniPlayer(song: curSong, colors: curColors, onTap: () => _openNowPlaying(kSongs, _curIdx),
...
class _SongsTab extends StatelessWidget {
  ...
  const _SongsTab({required this.filtered, required this.songs, required this.selMood,
    required this.searchCtrl, required this.curIdx, required this.player,
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: ListView.builder(padding: const EdgeInsets.symmetric(horizontal: 12),
          ...
          itemBuilder: (_, i) {
            ...
            final playing = curIdx == idx;
            final mc = mood(s['mood']!);
            return Padding(padding: const EdgeInsets.only(bottom: 8),
                child: GestureDetector(onTap: () => onTap(idx, s), onLongPress: () => onLongPress(s),
...
class _PlaylistsTab extends StatelessWidget {
  ...
  const _PlaylistsTab({required this.playlists, required this.player,
    required this.onCreate, required this.onDelete, required this.onPlay});
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: playlists.isEmpty
        ...
        Icon(Icons.queue_music_rounded, color: Colors.white24, size: 64),
        SizedBox(height: 16),
        Text('No playlists yet', style: TextStyle(color: Colors.white38, fontSize: 16, fontWeight: FontWeight.w600)),
        SizedBox(height: 8),
        Text('Tap "+ New Playlist" to create one', style: TextStyle(color: Colors.white24, fontSize: 13))]))
            ...
            return Padding(padding: const EdgeInsets.only(bottom: 10),
                child: GestureDetector(
                    onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) =>
                        PlaylistDetailPage(playlist: pl, onPlay: onPlay))),
                          ...
                          ClipRRect(borderRadius: BorderRadius.circular(12),
                              child: SizedBox(width: 56, height: 56,
                                  child: pl.songs.isEmpty
                                      ...
                                      child: const Icon(Icons.queue_music_rounded, color: Colors.white30, size: 28))
                                      : GridView.count(crossAxisCount: 2,
                                      physics: const NeverScrollableScrollPhysics(),
                                      children: List.generate(4, (gi) => gi < pl.songs.length
                                          ? Image.asset(pl.songs[gi]['art']!, fit: BoxFit.cover)
                                          : Container(color: Colors.white10))))),
                          const SizedBox(width: 14),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class PlaylistDetailPage extends StatefulWidget {
  ...
  final void Function(List<Map<String, String>>, int) onPlay;
  const PlaylistDetailPage({super.key, required this.playlist, required this.onPlay});
  @override State<PlaylistDetailPage> createState() => _PlaylistDetailPageState();
}

class _PlaylistDetailPageState extends State<PlaylistDetailPage> {
  void _remove(int i) => setState(() => widget.playlist.songs.removeAt(i));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: const Color(0xFF0A0A0A),
        appBar: AppBar(backgroundColor: Colors.transparent, elevation: 0,
            ...
            title: Text(pl.name, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 18)),
            centerTitle: true),
        body: pl.songs.isEmpty
            ? const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.music_off_rounded, color: Colors.white24, size: 64),
          SizedBox(height: 16),
          Text('No songs in this playlist', style: TextStyle(color: Colors.white38, fontSize: 16)),
          SizedBox(height: 8),
          Text('Long press any song to add it', style: TextStyle(color: Colors.white24, fontSize: 13))]))
            ...
            itemBuilder: (_, i) {
              ...
              final mc = mood(s['mood']!);
              return Padding(padding: const EdgeInsets.only(bottom: 8),
                  child: Dismissible(key: ValueKey('${pl.id}_${s['title']}_$i'),
                      ...
                      background: Container(alignment: Alignment.centerRight, padding: const EdgeInsets.only(right: 20),
                          decoration: BoxDecoration(color: Colors.redAccent.withValues(alpha: 0.2),
                              borderRadius: BorderRadius.circular(16)),
                          child: const Icon(Icons.delete_rounded, color: Colors.redAccent)),
                      child: GestureDetector(onTap: () => widget.onPlay(pl.songs, i),
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  StreamBuilder<Duration>(stream: globalPlayer.positionStream, builder: (_, snap) {
                    ...
                    return Column(children: [
                      SliderTheme(data: SliderThemeData(
                          ...
                          trackHeight: 2, thumbColor: Colors.white,
                          ...
                          child: Slider(min: 0,
                              ...
                              onChanged: (v) async {
                                await globalPlayer.seek(Duration(milliseconds: v.toInt()));
                                if (!globalPlayer.playing) await globalPlayer.play();
                              })),
                      Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
                          child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                            Text(_fmt(pos), style: const TextStyle(color: Colors.white60, fontSize: 10)),
                            Text(_fmt(dur), style: const TextStyle(color: Colors.white60, fontSize: 10)),
                          ])),
                    ]);
                  }),
                  const SizedBox(height: 4),
                  Row(children: [
                    ClipRRect(borderRadius: BorderRadius.circular(8),
                        child: Image.asset(song['art']!, width: 42, height: 42, fit: BoxFit.cover)),
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text(song['artist']!, style: const TextStyle(color: Colors.white60, fontSize: 11)),
                    ])),
                    ...
                    StreamBuilder<PlayerState>(stream: globalPlayer.playerStateStream, builder: (_, snap) {
...
class NowPlayingPage extends StatefulWidget {
  ...
  final int globalIdx;
  final void Function(int globalIdx) onIndexChanged;
...
class _NowPlayingPageState extends State<NowPlayingPage>
  ...
  void initState() {
    ...
    // Sync NowPlaying page if autoplay triggers from global player
    globalPlayer.playerStateStream.listen((state) {
  ...
  Future<void> _playAt(int listIdx) async {
    ...
    // Notify home page of new global index
    final gi = kSongs.indexWhere((k) => k['title'] == s['title']);
    if (gi != -1) widget.onIndexChanged(gi);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(children: [
        ...
        // PARTICLES — always visible in now playing
        ...
        SafeArea(child: Padding(
          ...
          child: Column(children: [
            ...
            // ALBUM ART — bounce + slide on song change
                    ...
                    child: ScaleTransition(scale: _artScale,
                        child: Container(
                            ...
                            child: ClipRRect(borderRadius: BorderRadius.circular(28),
                                child: Image.asset(_song['art']!, width: 290, height: 290, fit: BoxFit.cover))))))),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                AnimatedSwitcher(duration: const Duration(milliseconds: 250),
                    child: Text(_song['title']!, key: ValueKey(_song['title']),
              ...
              AnimatedContainer(duration: const Duration(milliseconds: 400),
                  ...
                  decoration: BoxDecoration(color: _colors[0].withValues(alpha: 0.35),
            ...
            StreamBuilder<Duration>(stream: globalPlayer.positionStream, builder: (_, snap) {
              ...
              return Column(children: [
                ...
                Padding(padding: const EdgeInsets.symmetric(horizontal: 8),
                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      Text(_fmt(pos), style: const TextStyle(color: Colors.white60, fontSize: 12)),
            ...
            StreamBuilder<PlayerState>(stream: globalPlayer.playerStateStream, builder: (_, snap) {
              ...
              return Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                // PREV
                _CircleBtn(icon: Icons.skip_previous_rounded, size: 30, onTap: _prev),

                // PLAY/PAUSE — big pulsing button
                GestureDetector(onTap: () => playing ? globalPlayer.pause() : globalPlayer.play(),
                    child: AnimatedContainer(duration: const Duration(milliseconds: 200),
                        width: playing ? 76 : 72, height: playing ? 76 : 72,
                        decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle,
                            boxShadow: [BoxShadow(color: Colors.white.withValues(alpha: playing ? 0.45 : 0.2),
                                blurRadius: playing ? 32 : 16, spreadRadius: playing ? 4 : 1)]),
                        child: Icon(playing ? Icons.pause_rounded : Icons.play_arrow_rounded,
                            color: _colors[0], size: 40))),

                // NEXT
                _CircleBtn(icon: Icons.skip_next_rounded, size: 30, onTap: _next),
              ]);
            }),
